...
2.  FastAPI receives request (`user_id`, `message`, `session_id`, `organization_id`).
3.  `RunnableWithMessageHistory` loads history.
...
10. `SQLQueryTool` generates and runs SQL (using resolved IDs). Results (borrow counts) added to state.
