...
async def async_tools_node_handler(state: AgentState, tools: List[Any]) -> Dict[str, Any]:
    ...
    # --- Processing results: Now only focuses on tables/visualizations ---
    # The hierarchy_name_resolver results are just passed back in the ToolMessage content for the agent LLM
    new_tables = []
    new_visualizations = []
    new_messages = []
    ...
    for result in results:
        if result["success"]:
            ...
            raw_content = result.get("raw_content") # Use the raw output for parsing
            new_messages.append(tool_message) # Always append the message

            # Only try to parse tables/visualizations from specific tools
            if tool_name in ["sql_query", "chart_renderer"]:
                content_to_parse = None
                if isinstance(raw_content, dict):
                    content_to_parse = raw_content
                    logger.debug(f"Raw content for tool {tool_name} is dict.")
                elif isinstance(raw_content, str):
                    content_str = raw_content.strip()
                    if content_str.startswith('{') and content_str.endswith('}'):
                        try:
                            content_to_parse = json.loads(content_str)
                            logger.debug(f"Successfully parsed raw string content for tool {tool_name}")
                        except json.JSONDecodeError as json_err:
                            logger.warning(f"Failed to parse raw string content as JSON for tool {tool_name}: {json_err}. String was: {content_str[:100]}...")
                    else:
                         logger.debug(f"Raw string content for tool {tool_name} is not JSON-like.")
                else:
                    logger.warning(f"Raw content for tool {tool_name} is neither dict nor str: {type(raw_content)}")

                if content_to_parse:
                    try:
                        # Extract table from SQL results
                        if tool_name == "sql_query" and "table" in content_to_parse and content_to_parse["table"]:
                            ...
                            if isinstance(table_data, dict) and 'columns' in table_data and 'rows' in table_data:
                                # Duplicate check (basic version) - consider improving if needed
                                already_exists = False
                                existing_tables = state.get("tables", [])
                                for existing_table in existing_tables:
                                    if existing_table == table_data: # Simple comparison
                                        already_exists = True
                                        logger.warning(f"Table data from tool '{tool_name}' seems duplicate. Skipping append.")
                                        break
                                if not already_exists:
                                    logger.info(f"Extracted table from sql_query: {len(table_data.get('rows', []))} rows. Appending.")
                                    new_tables.append(table_data)
                            else:
                                logger.warning(f"'table' key found in sql_query result, but content is not valid table dict: {type(table_data)}")

                        # Extract visualization from chart renderer
                        if tool_name == "chart_renderer" and "visualization" in content_to_parse and content_to_parse["visualization"]:
                            viz_data = content_to_parse["visualization"]
                            if isinstance(viz_data, dict) and 'type' in viz_data and 'image_url' in viz_data:
                                logger.info(f"Extracted visualization: {viz_data.get('type', 'unknown')} chart")
                                new_visualizations.append(viz_data)
                            else:
                                logger.warning(f"'visualization' key found in chart_renderer result, but content not valid viz dict: {type(viz_data)}")

                    except Exception as e:
                        logger.error(f"Error processing extracted data from tool '{tool_name}' result: {str(e)}")
            # else: No special processing needed for hierarchy_name_resolver output here
        else:
            # Handle failed execution
            error_msg = result["error"]
            tool_id = result.get("tool_id", "")
            tool_name = result.get("tool_name", "unknown_tool")
            # Create a ToolMessage indicating failure
            new_messages.append(ToolMessage(
                content=f"Tool execution failed: {error_msg}",
                tool_call_id=tool_id,
                name=tool_name # Include tool name in error message
            ))

    # Return updates for LangGraph to merge using operator.add
    update_dict = {
