...
- Automatic result limiting (max 50 rows per query)
- Robust error handling with specific error types
- Production-ready optimizations for large datasets
- **Graph-based agent execution for controlled, reliable workflow**
...
- Hybrid final response formatting
...
```json
{
  "status": "success",
  "data": {
    "text": "Here is the chart:", // Final text response (concise if viz is present)
    "table": { // Table data if relevant and no visualization
      "columns": ["day", "count"],
      "rows": [
        ["Monday", 32],
        ["Tuesday", 28],
    ...
    "visualization": { // Visualization data if generated
      "type": "bar",
      "image_url": "/static/charts/chart_some_uuid.png" // URL path to PNG
    } | null
  },
  "error": null,
  "timestamp": "..."
...
2. Implement `_run` / `_arun`. Ensure output is structured (e.g., `dict` or basic types) and ideally JSON-serializable. Dictionaries from tools will be JSON stringified before being added to `FunctionMessage`.
